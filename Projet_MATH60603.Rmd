...
train$validation = 0
train$validation= runif(nrow(train), min=0, max=100)
train$validation = 0

train <- train[order(train$validation),]
train$validation[1:nrow(test)] =1


model1=glm(Churn~ . - validation, family="binomial",data=train[train$validation==0,] )
summary(model1)
...
library(ROCR)
prob.predict=predict.glm(model1,train[train$validation==1,],type="response")
cutoff=0.5
test.pred = rep(0, nrow(mydata[train$validation==1,]))
test.pred[prob.predict > cutoff] = 1
M=table(test.pred, mydata[train$validation==1,]$Churn,dnn=c("Prediction","Observation"))
...
train = subset(train, select = -c(validation) )
...
myboost=boosting(Churn~., data=mydata, mfinal = 100, coeflearn = 'Freund', control=rpart.control(maxdepth=10))
myboost$importance
...
# Taux faux positifs arbre élagé
round((1-(a/(a+c))), digit = 2 ) 

# Taux faux négatif arbre élagé
round((1-(d/(b+d))), digit = 2)

# Taux de mauvaises classifications arbre élagé
((b+c)/(a+b+c+d))*100
