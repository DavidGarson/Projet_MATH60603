...
L'objectif de cette étude est de faire une prédiction sur le désbonnement d'un client pour un service de télécommunication. Cette simulation a été fait à partir d'une base de données présent sur kaggle à l'adresse suivante :
<br>
https://www.kaggle.com/mnassrib/telecom-churn-datasets?select=churn-bigml-80.csv. 
<br>
L'analyse a été effectué sur les client d'Orange Telecom aux États-Unis et plusieurs variables on été analysés tels que lieu géographique, le type de plan que le client possède, le nombre de minutes utilisé pour des appels durant le jour et le soir, etc. 
<br>
<br>
Ce rapport contient en premier lieu, une présentation détaillé de notre jeu de donnée : présentation des variables et des observations, quelques analyses de corrélations entre les variables et de traintement et réencodage préalable du jeu de donnée. 
<br>
Ensuite, plusieurs méthode et stratégie de prédiction seront testés afin de déterminer le modèle le mieux adapté à notre échantillion d'utilisateurs. Tels que la régressions, les abres et forêt élague, etc. 
<br>
Finalement, les caractéristiques de chacun des modèeles seront comparés afin de déterminer le meilleur modèele de prédiction pour notre jeu de donnée.  
...
mydata$Churn[mydata$Churn == "False"] = 0
mydata$Churn[mydata$Churn == "True"] = 1
mydata$Churn = as.numeric(mydata$Churn)
mydata$Churn=as.logical(mydata$Churn)


mydata$International.plan[mydata$International.plan == "No"] = 0
mydata$International.plan[mydata$International.plan == "Yes"] = 1
mydata$International.plan = as.numeric(mydata$International.plan)
mydata$International.plan=as.logical(mydata$International.plan)

mydata$Voice.mail.plan[mydata$Voice.mail.plan == "No"] = 0
mydata$Voice.mail.plan[mydata$Voice.mail.plan == "Yes"] = 1
mydata$Voice.mail.plan = as.numeric(mydata$Voice.mail.plan)
mydata$Voice.mail.plan=as.logical(mydata$Voice.mail.plan)
...
Cette section présentera plusieurs simulations pour porédire la valeur du churn selon les variables de notre jeu de donnée. Les simulations qui serons testés sont les suivantes : 
...
model1=glm(Churn~ ., family="binomial",data=trainRL)

#BIC  
BIC = round(BIC(model1), digit = 2)

#AIC
AIC = round(model1$aic, digit = 2)

library(ROCR)
prob.predict=predict.glm(model1,testRL,type="response")
cutoff=0.5
test.pred = rep(0, nrow(testRL))
test.pred[prob.predict > cutoff] = 1
M=table(test.pred, testRL$Churn,dnn=c("Prediction","Observation"))
...
pred=prediction(prob.predict,testRL$Churn)
